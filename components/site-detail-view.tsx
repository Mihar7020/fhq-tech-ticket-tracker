  ...
  return (
    <div className="page-wrap">
      ...
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="card p-5"><p className="label">Total requests</p><p className="display mt-4 text-4xl">{tickets.length}</p><p className="muted mt-1 text-[10px]">All recorded tickets</p></div>
      </section>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
        ...
        <section className="card p-5">
          ...
          <div className="mt-5 space-y-4">
            {Array.from(new Map(tickets.filter((ticket) => ticket.category !== "Uncategorized").map((ticket) => [ticket.category, tickets.filter((item) => item.category === ticket.category).length])).entries()).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([issue, count], index) => {
            ...
            {!tickets.length && <p className="muted text-sm">Patterns will appear after tickets are recorded.</p>}
          </div>
        </section>
      </div>
    </div>
