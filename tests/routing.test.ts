...
describe("routing resolution", () => {
  it("applies an exact email alias with stable person and site", () => { const result = resolveRoute({ senderEmail: " TWHITEHORSE@FHQTC.NET ", people, sites }); expect(result.outcome).toBe("exact"); expect(result.personId).toBe("p1"); expect(result.siteId).toBe("sbec"); });
  it("uses a unique site domain as a strong site signal", () => { const result = resolveRoute({ senderEmail: "new.teacher@okanese.edu", people, sites }); expect(result.outcome).toBe("domain"); expect(result.siteId).toBe("ok"); });
  ...
  it("flags conflicting directory and body site signals", () => { const result = resolveRoute({ senderEmail: "tara.whitehorse@standingbuffalo.edu", body: "I am at Okanese School today.", people, sites }); expect(result.outcome).toBe("conflict"); expect(result.signals.length).toBeGreaterThan(1); });
