...
describe("ingest pipeline", () => {
  ...
  it("preserves original CC recipients", async () => { const repo = createMemoryRepository({ people, sites }); await ingestMime(mime({ id: "<cc>", cc: "Principal <principal@example.org>" }), repo); expect(repo.records[0].ccRecipients).toEqual(["principal@example.org"]); });
});
