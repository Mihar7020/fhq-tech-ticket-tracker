...
export const prismaIngestRepository: IngestRepository = {
  ...
  async listThreadMessages() {
    const rows = await db.message.findMany({ select: { internetMessageId: true, subject: true, threadId: true, fromAddress: true, sentAt: true, thread: { select: { externalThreadId: true } } }, where: { threadId: { not: null } }, orderBy: { sentAt: "desc" }, take: 20_000 });
    return rows.map((row) => ({ messageId: row.internetMessageId, subject: row.subject, threadId: row.threadId!, externalThreadId: row.thread?.externalThreadId ?? undefined, senderEmail: row.fromAddress, sentAt: row.sentAt }));
  },
  async save(record: IngestRecord) {
    return db.$transaction(async (tx) => {
      if (record.threadMatch.kind === "match") {
        ...
        await tx.emailThread.update({ where: { id: thread.id }, data: { lastMessageAt: record.sentAt, weldedMessageCount: { increment: 1 }, externalThreadId: thread.externalThreadId ?? record.externalThreadId } });
      ...
      const thread = await tx.emailThread.create({ data: { ticketId: ticket.id, normalizedSubject: record.normalizedSubject, externalThreadId: record.externalThreadId, lastMessageAt: record.sentAt } });
